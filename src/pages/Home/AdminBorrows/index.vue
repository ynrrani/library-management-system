...
import {deleteBorrow,searchBorrow} from '@/api'
import qs from 'qs'
export default {
    name: 'AdminBorrows',
    data(){
  ...
  computed:{
    ...
    methods:{
        handleDelete(index,row){
            console.log(row);
            let borrowObj = {readerId:row.readerId,bookId:row.bookId,borrowDate:row.borrowDate}
            deleteBorrow(qs.stringify(borrowObj)).then(res=>{
                console.log(res);
        ...
        searchInfo(e){
              this.loading = true
              searchBorrow(qs.stringify({info:this.info})).then(res=>{
              this.loading = false
              e.target.blur()
              console.log(res);
