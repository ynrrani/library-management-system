    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        <el-popover trigger="hover" placement="top">
          <p>姓名: {{ !scope.row.readerName ? scope.row.readerId : scope.row.readerName }}</p>
          <div slot="reference" class="name-wrapper">
            <el-tag size="medium">{{ !scope.row.readerName ? scope.row.readerId : scope.row.readerName }}</el-tag>
          </div>
        </el-popover>
      </template>
    </el-table-column>
    ...
    <el-table-column label="审核状态">
      <template slot-scope="scope">
        <el-button
          size="mini"
          disabled
          slot="reference">{{scope.row.status}}</el-button>
